...
-- 코멘트
COMMENT ON TABLE pricing_policies IS '시간대별 예약 가격 정책 (PricingPolicy Aggregate Root)';
COMMENT ON COLUMN pricing_policies.room_id IS '룸 ID (외부 서비스 참조)';
COMMENT ON COLUMN pricing_policies.place_id IS '플레이스 ID (외부 서비스 참조)';
COMMENT ON COLUMN pricing_policies.day_of_week IS '요일 (MONDAY~SUNDAY)';
COMMENT ON COLUMN pricing_policies.start_time IS '시작 시간';
COMMENT ON COLUMN pricing_policies.end_time IS '종료 시간';
COMMENT ON COLUMN pricing_policies.price IS '가격 (원)';
...
-- 코멘트
COMMENT ON TABLE products IS '추가상품 (Product Aggregate Root)';
COMMENT ON COLUMN products.name IS '상품명';
COMMENT ON COLUMN products.description IS '상품 설명';
COMMENT ON COLUMN products.price IS '상품 가격 (원)';
COMMENT ON COLUMN products.scope_type IS '적용 범위 (PLACE/ROOM/RESERVATION)';
COMMENT ON COLUMN products.scope_id IS '적용 범위 ID (플레이스/룸 ID 등)';
COMMENT ON COLUMN products.total_quantity IS '총 재고 수량';
COMMENT ON COLUMN products.used_quantity IS '사용된 수량';
COMMENT ON COLUMN products.is_active IS '활성 상태';
...
-- 코멘트
COMMENT ON TABLE reservation_pricings IS '예약 가격 스냅샷 (ReservationPricing Aggregate Root)';
COMMENT ON COLUMN reservation_pricings.reservation_id IS '예약 ID (외부 서비스 참조, Unique)';
COMMENT ON COLUMN reservation_pricings.room_id IS '룸 ID';
COMMENT ON COLUMN reservation_pricings.place_id IS '플레이스 ID';
COMMENT ON COLUMN reservation_pricings.base_price IS '기본 가격 (룸 가격)';
COMMENT ON COLUMN reservation_pricings.total_price IS '총 가격 (기본 가격 + 추가상품)';
...
-- 코멘트
COMMENT ON TABLE reservation_pricing_items IS '예약 가격 항목 (추가상품 스냅샷, Entity)';
COMMENT ON COLUMN reservation_pricing_items.reservation_pricing_id IS '예약 가격 ID (FK)';
COMMENT ON COLUMN reservation_pricing_items.product_id IS '상품 ID (스냅샷 시점)';
COMMENT ON COLUMN reservation_pricing_items.product_name IS '상품명 (스냅샷)';
COMMENT ON COLUMN reservation_pricing_items.product_price IS '상품 단가 (스냅샷)';
COMMENT ON COLUMN reservation_pricing_items.quantity IS '수량';
COMMENT ON COLUMN reservation_pricing_items.item_total_price IS '항목 총 가격 (단가 x 수량)';
