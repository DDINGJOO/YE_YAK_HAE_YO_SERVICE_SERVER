...
import com.teambind.springproject.adapter.in.messaging.kafka.handler.ReservationCancelledEventHandler;
...
@DisplayName("ReservationCancelledEventHandler 단위 테스트")
class ReservationCancelledEventHandlerTest {
	
	@Mock
	private ReservationPricingRepository reservationPricingRepository;
	
	@InjectMocks
	private ReservationCancelledEventHandler handler;
	
	private ReservationCancelledEvent event;
	...
	@Nested
	@DisplayName("handle() - 예약 취소 이벤트 처리")
	class HandleTests {
		
		@Test
		@DisplayName("PENDING 상태의 예약을 CANCELLED로 변경한다")
	...
	class GetSupportedEventTypeTests {
		...
		@Test
		@DisplayName("ReservationCancelled 이벤트 타입을 반환한다")
		void returnsSupportedEventType() {
			// when
			final String eventType = handler.getSupportedEventType();
			
			// then
			assert eventType.equals("ReservationCancelled");
		}
	}
}
